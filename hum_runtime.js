 ...
 * requires: core.js, actor,js, humus.js, hum_xlat.js
 */
...
if (typeof DALNEFRE.Humus.Runtime !== 'undefined') {
	throw Error('Module "DALNEFRE.Humus.Runtime" already defined!');
}
...
DALNEFRE.Humus.Runtime = (function () {
	...
	var factory;
	var constructor = function Runtime(generator) {  // e.g.: DALNEFRE.Humus.Gen_Meta
		var cfg = DAL.Actor.Config();
		var GEN = generator(cfg);
		var Actor = GEN.Actor;
		var XLAT = HUM.Xlat(GEN);
		var env;
		var println_beh = function (id) {
			id = id || 'output';
			return function (msg) {
				DAL.println(DAL.id(id), HUM.pp(msg));
			};
		};
...
//		var a_logger = Actor(sink_beh, 'logger');
		var a_logger = Actor(println_beh('debug'), 'logger');
		var a_config = Actor(GEN.config_beh(a_logger), 'config');
		var a_timer = Actor(timer_beh(a_config), 'timer');
		var d_timer = Actor(GEN.upcall_beh(a_timer), '^timer');
		var d_random = Actor(GEN.upcall_beh(
			Actor(random_beh(a_config), 'random')
		), '^random');
		var a_println = Actor(println_beh('output'), 'println');
		var d_println = Actor(GEN.upcall_beh(a_println), '^println');
		var a_sponsor = Actor(GEN.repl_sponsor_beh(a_config, a_println), 'sponsor');
		var env = Actor(GEN.empty_env_beh, 'empty_env');
/*		DAL.log('initializing environment'); */
		env = Actor(GEN.repl_env_beh({
			'versions':
			...
			'sink':
				Actor(GEN.upcall_beh(cfg.sink), '^sink'), 
			'random': d_random,
			'timer': d_timer,
			'println': d_println,
			'map_empty': f_map_empty,
			'map_bind':
				Actor(GEN.native_fn_beh(map_bind_fn), 'map_bind_fn'),
			'tuple_to_symbol':
				Actor(GEN.native_fn_beh(tuple_to_symbol_fn), 'tuple_to_symbol_fn'),
			'tuple_to_number':
				Actor(GEN.native_fn_beh(tuple_to_number_fn), 'tuple_to_number_fn'),
			'in_range_set':
				Actor(GEN.native_fn_beh(in_range_set_fn), 'in_range_set_fn'),
			'less':
				Actor(GEN.native_fn_beh(less_fn), 'less_fn'),
			'less_equal':
				Actor(GEN.native_fn_beh(less_equal_fn), 'less_equal_fn'),
			'greater_equal':
				Actor(GEN.native_fn_beh(greater_equal_fn), 'greater_equal_fn'),
			'greater':
				Actor(GEN.native_fn_beh(greater_fn), 'greater_fn'),
			'not':
				Actor(GEN.native_fn_beh(not_fn), 'not_fn'),
			'or':
				Actor(GEN.native_fn_beh(or_fn), 'or_fn'),
			'and':
				Actor(GEN.native_fn_beh(and_fn), 'and_fn'),
			'neg':
				Actor(GEN.native_fn_beh(neg_fn), 'neg_fn'),
			'div':
				Actor(GEN.native_fn_beh(div_fn), 'div_fn'),
			'sub':
				Actor(GEN.native_fn_beh(sub_fn), 'sub_fn'),
			'mul':
				Actor(GEN.native_fn_beh(mul_fn), 'mul_fn'),
			'add':
				Actor(GEN.native_fn_beh(add_fn), 'add_fn'),
			'eq':
				Actor(GEN.native_fn_beh(eq_fn), 'eq_fn'),
			'compare':
